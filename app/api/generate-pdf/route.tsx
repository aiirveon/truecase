...
import { stripSroiCaveat } from '@/lib/calculations'
...
const s = StyleSheet.create({
  ...
  // Tighter body — used for the Section 2 narrative on the governance page so
  // the narrative plus the six-row table fit on a single physical page.
  bodyTight: {
    fontSize: 10,
    lineHeight: 1.5,
    marginBottom: 8,
...
function Footer({ date }: { date: string }) {
  return (
    // `fixed` renders the footer at the bottom of every physical page this
    // <Page> produces, so an overflow never strands a page without a footer.
    <View style={s.footer} fixed>
      <View style={s.footerRow}>
        <Text style={s.footerMain}>
          Generated by TrueCase — truecase-seven.vercel.app
        </Text>
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.bodyTight}>{section2}</Text>

        {/* Governance table — wrap={false} keeps all six rows together on
            this page rather than splitting and stranding rows onto a 4th page */}
        <View style={s.table} wrap={false}>
          ...
          {governanceElements.map((el, i) => {
            ...
            return (
              <View key={el.element_id} style={rowStyle} wrap={false}>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.body}>{stripSroiCaveat(section3)}</Text>
